...
if(isset($_POST[submit]))
{
	if(isset($_SESSION[doctorid]))
	{
		...
		{
			echo "<script>alert('Vet profile updated successfully...');</script>";
		}
		else
		{
	...
	else
	{
		...
		if($qsql = mysqli_query($con,$sql))
		{
			echo "<script>alert('Vet record inserted successfully...');</script>";
		}
		else
		{
			echo mysqli_error($con);
...
<div class="container-fluid">
    <div class="block-header">
        <h2 class="text-center"> Vet's Profile</h2>
    </div>
...
function validateform() {
    if (document.frmdoctprfl.doctorname.value == "") {
        ...
        alert("Bull Catalog ID should not be empty..");
